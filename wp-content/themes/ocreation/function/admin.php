<?php
/**
 *
 * Fonctions admin
 * Décommenter les fonctions pour les utiliser
 *
 * Si la fonction n'existe pas :
 * 1) Créer un fichier dans le répertoire admin/ en snake_case
 * 2) Require la fonction dans la partie custom de ce fichier
 *
 */

require_once('admin/add_theme_support.php');
require_once('admin/yoast.php');
require_once('admin/compte-client.php');
require_once('admin/profil.php');
require_once('admin/remove_tag_support.php');
//require_once('admin/page_option_acf.php');
//require_once('admin/wpadmin_filter.php');


/**
 * Custom theme
 */
